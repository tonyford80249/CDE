...
<script type="text/javascript">
   function calculateOtherFields(index) {
	  		...
	  		//alert(document.getElementById('totalPurchasePrice' + index).value);
	  		//if (''== document.getElementById('totalPurchasePrice' + index).value)
	  		//	return;
   ...
   function calculateMilesRelatedFields(index) {
	   ...
	   var i = 0;
		  ...
		  // document.getElementById('capitalOutlayGridList' + index + '.contractorMiles').value = document.getElementById('capitalOutlayGridList' + index + '.contractorMiles').value + '.00%';
		   /*var totalFunding = 0;
		    
			for (var j=0; j<10; j++) {
				exclValue =  parseFloat(document.getElementById('capitalOutlayGridList' + j + '.outlayExclusion').value.replace(/[^0-9-.]/g));
				if (isNaN(exclValue))
					continue;
				totalFunding = totalFunding * 1 + exclValue;
			}*/
	   ...
	   for (var j=0; j<10; j++) {
		   ...
		   if (document.getElementById('totalPurchasePrice' + j) == null || ''== document.getElementById('totalPurchasePrice' + j).value )
			...
			document.getElementById('totalPurchasePrice' + j).value = (document.getElementById('totalPurchasePrice' + j).value).replace(/[^0-9-.]/g, '');
		   	...
		   	//alert(document.getElementById('capitalOutlayGridList' + j + '.totalPurchasePrice').value);
	   }
   }
...
<form:form commandName="capitalOutlayForm"
  ...
  <div class="formBlock boxShadow3 radius10">
	 ...
	 <div style='float: left;'>
	  <span class="field" style='width: 100px;'><label for="preparerName">Name</label></span>
	  <span class="bodyText" style='width: 350px;'>
		<form:input id="" path="preparerName" size="25"
									maxlength="100" />
	  </span>
	  <br/>
	  <span class="field" style='width: 100px;'><LABEL FOR=""><spring:message
										code="CapitalOutlay.StaticText.emailAddressStaticText" /></LABEL>
	  </span>
	  <span class="bodyText" style='width: 350px;'>
	  <form:input id="" path="preparerEmailId" size="40"
									maxlength="100"  onBlur="validateEmail('preparerEmailId')"/>
	  </span>
	  <br/>
	  <span class="field" style='width: 100px;'><label for="preparerPhoneNos">Phone</label></span>
	  <span class="bodyText" style='width: 350px;'>
	    <form:input id="" path="preparerPhoneNos" size="16"
									maxlength="16" onBlur="validatePhoneNos('preparerPhoneNos')"/>
	  </span>
	</div>
	<div style='float: left; background-color: #fafafa; border: 1px solid #ddd; padding: 0.5em 1em;' class='radius5'>
	  ...
	  <span class="bodyText">
	     <form:input id="DistrictName"
									path="districtName" size="26" maxlength="26" />
	  </span>
	</div>
		...
		<table id="capitalOutlayGrid" class="display">
			<thead>
				<tr>
					...
					<th width="6%">Total Purchase Price for all Vehicles for Year</th>
					...
					<th width="6%">Percentage of Contractor Miles for the District</th>
			...
			<tbody>
				...
				<tr>
					...
					<td width="4%">
					  <form:hidden	path="capitalOutlayGridList[${idx.index}].id" />
				      <form:hidden	path="capitalOutlayGridList[${idx.index}].relatedYear" />
					  <c:out value="${capitalOutlayForm.capitalOutlayGridList[idx.index].relatedYear}" />
					</td>
					<td width="6%"><form:input
												...
												size="10" maxlength="10"
													...
													<td width="6%"><form:input readonly="true"
															size="10" maxlength="10"
															id="depreciationCycleValue${idx.index}"
													...
													<td width="6%"><form:input size="8" maxlength="8"
															...
															onBlur="calculateMilesRelatedFields(${idx.index})"
															/></td>
  ...
  <div style='margin: 1em;'>
	...
	<form:input id="capitalOutlayTotalFunding"
									path="capitalOutlayTotalFunding" size="10" maxlen="10"/>
  </div>

  <div style='text-align: center; margin: 1em 0;'>
	<input type="Submit" id="saveButton" value="Save/Electronic Signature" onClick="removeFormatting()" />
  </div>
